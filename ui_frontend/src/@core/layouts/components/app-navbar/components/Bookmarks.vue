...
import {
  // BNavItem, BTooltip,
  BNavbarNav, BNavItemDropdown, BFormInput, BDropdownItem,
} from 'bootstrap-vue'
...
export default {
  //  BNavItem, BTooltip
  components: {
    BNavbarNav, BNavItemDropdown, BFormInput, VuePerfectScrollbar, BDropdownItem,
  },
  setup() {
    ...
    const onshowing = async () => {
      ...
      bookmarks.value = temp.data.filter(page => page.isBookmarked)
    }
    const {
      searchQuery,
      resetsearchQuery,
      filteredData,
    } = useAutoSuggest({ data: { pages: store.state.appNavigation }, searchLimit: 6 })
    // searchAndBookmarkDataPages.value
    ...
    const toggleBookmarked = async item => {
      ...
      if (pageIndexInBookmarks > -1) {
        ...
        await axios.delete('/api/user-favorites/user-favorite-menu', {
          params: {
        ...
        await axios.put('/api/user-favorites/user-favorite-menu', {
          path: item.route.name,
          userId: window.sessionStorage.getItem('userId'),
        })
      }
